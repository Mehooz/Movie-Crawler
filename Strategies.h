...
public:
    BaseStrategy();
    virtual void exec(std::vector<BaseData*>& complexData,std::vector<BaseData*>& simpleData)=0;
};

class Imdb_movies_Strategy: public BaseStrategy{
 public:
    void exec(std::vector<BaseData*>& complexData,std::vector<BaseData*>& simpleData);
};
